...
def slide_window(image, image_address, x_begin: int, y_begin: int, w: int, h: int, window_size: int, stride: int,
                   contours, points_con_thre, area_ratio_thre,
                 svs_address):
    """通过滑窗的方式得到小窗口
    :param image: svs文件
    ...
    logger.info([x_begin,y_begin,x_end,y_end])
    wsi = wsireader.OpenSlideWSIReader(svs_address)
    ...
    for x in range(x_begin, x_end, stride):
        for y in range(y_begin, y_end, stride):
            ...
            if ratio < area_ratio_thre:
                ...
                ret.save(image_address+"_"+str(i_name)+".orig.png")
                # misc.imwrite(image_address+"_"+str(i_name)+".orig.png", target_area_img)
