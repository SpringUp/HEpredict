...
def generative_model(config, model, k, cnv=False):

    image_datasets = {x: CustomDset(os.getcwd()+f'/lung_data/{x}_{k}.csv',
    ...
    if cnv:
        model_ft = Cnn_With_cnv_Net(model_ft)
    else:
    ...
    try:
        ...
        print('failed to torch.nn.parallel.DistributedDataParallel for model_ft due to {}'.format(e))
    ...
    model_ft, tb = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler, dataloaders, 
        dataset_sizes, config.device, num_epochs=30, cnv=False)#调用train_model（train）
    tb.close()

    if k < 0:
        save_model = os.getcwd()+f'/results/models_resnet50/SYNE1/{model}'
    else:
        save_model = os.getcwd()+f'/results/models_resnet50/SYNE1/{model}_{k}'
    if cnv:
...
def main(config, ocs, classification, K, cnv, isCrossValidation):
    ...
    if isCrossValidation:
        for k in range(K):
            data_path = os.getcwd()+f"/data/SYNE1/train_{k}.csv"
            generative_model(config, "resnet50", k, cnv=False)
        ...
        data_path = os.getcwd()+f"/database2/train.csv"
        generative_model("resnet50", data_path, -11, cnv=cnv)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print('torch.cuda.set_device(args.local_rank) ... failed due to {}'.format(e))
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu") 
        world_size = 1
        local_rank = 0 
    ...
    main(config, origirn_classfication_set, args.classification, args.K, args.cnv, args.isCrossValidation)
