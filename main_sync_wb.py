...
# For multiple card on single machine, do not use MASTER_PORT.
...
def generative_model(config, model, k, cnv="", split_folder='halves', classification_num=2, batch_size=256, lr=0.001):
    image_datasets = {x: CustomDset(f'{split_folder}/{x}_{k}.csv',
    ...
    if cnv:
        model_ft = Cnn_With_Clinical_Net(model_ft, classification_num=classification_num)
    else:
        model_ft = Net(model_ft, classification_num=classification_num)
    ...
    optimizer_ft = optim.SGD(model_ft.parameters(), lr=lr, momentum=0.9)#优化器 model_ft.parameters()获取网络参数 SGD是随机梯度下降 momentum 动量加速
...
def main(config, ocs, classification_num, K, cnv, isCrossValidation, use_model='resnet18', split_folder='halves', batch_size=256, lr=0.001):
    print(isCrossValidation)
    if isCrossValidation:
        for k in range(K):
            generative_model(config, use_model, k, cnv=cnv, split_folder=split_folder, classification_num=classification_num, batch_size=batch_size, lr=lr)
        ...
        generative_model(use_model, data_path, -11, cnv=cnv, classification_num=classification_num)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--classification_num", type=int, default=2)
    ...
    parser.add_argument("--split_folder", type=str, default='halves')
    ...
    main(config, origirn_classfication_set, args.classification_num, args.K, args.cnv, \
        args.isCrossValidation, use_model=args.model, split_folder=args.split_folder, 
