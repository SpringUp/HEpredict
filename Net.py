...
class Cnn_With_Clinical_Net(nn.Module):
    def __init__(self , model, classification_num=2):
        ...
        # clinical feature
        self.clinical = nn.Linear(35, 35)#全连接层输入/输出size（临床数据的特征提取，输入特征为55维向量）

        # concat合并
        self.mcb = CompactBilinearPooling(128, 35, 128).cuda()#双线性池化，特征融合输入1/输入2/输出 size
        # self.concat = nn.Linear(128+55, 128)
        self.bn = nn.BatchNorm1d(128)#标准化
        self.relu = nn.ReLU(True)
        self.classifier = nn.Linear(128, classification_num)

    def forward(self, x, clinical_features):#反向传播（x为图像特征，clinical_features为临床特征）
        #print(x.size(2)) # 256,3,224
        x = self.conv(x)
        #print(x.shape)
        
        #x = x.view(-1,256)
        x = x.view(x.size(0), -1)#相当于reshape，-1表示不确定(((qianyixuexi)
...
class Net(nn.Module):#更改分类数
    def __init__(self, model, classification_num=2):
        ...
        if type(self.layer[-1]) == type(nn.Sequential()):
            ...
            self.feature = self.layer[-1].in_features
        self.linear = nn.Linear(self.feature, classification_num)
